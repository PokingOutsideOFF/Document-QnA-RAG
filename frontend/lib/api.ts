...
export async function deleteDocument(filename: string): Promise<void> {
  const res = await fetch(
    `${BACKEND}/documents/${encodeURIComponent(filename)}`,
    { method: "DELETE" },
  );
...
/**
 * WHY hit Ollama directly from browser (not via FastAPI backend)?
 *  This is a read only discovery call, we just want to know what models are installed.
 *  Going through FastAPI would add a proxy endpoint with no benefit. Ollama allows cross-origin
 *  requests by default, so the browser can call it directly at http://localhost:11434
 */
